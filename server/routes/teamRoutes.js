...
router.post("/create", registrationLimiter, async (req, res, next) => {
  try {
    ...
    const userMetadata = `${encryptedRoll}|${leaderCollege}|${encryptedPhone}|${leaderCourse}|${leaderBranch}|${leaderSection}|${leaderYear}|APPROVED`;
    ...
    let team;
    try {
      // 5. Create Team Record. The public verification code is stored with
      // the team so it remains available after a server restart.
      ...
      // Do not leave a partial registration when any follow-up write fails.
      try {
        if (team?.id) await supabaseService.deleteTeam(team.id);
...
router.post("/join", registrationLimiter, async (req, res, next) => {
  try {
    ...
    const userMetadata = `${encryptedRoll}|${college}|${encryptedPhone}|${course}|${branch}|${section}|${year}|PENDING`;
    ...
    try {
      ...
      member = await supabaseService.addTeamMember({
        team_id: team.id,
        user_id: user.id,
        role: "MEMBER",
        status: "ACTIVE",
      ...
      try {
        ...
        console.error("Join rollback failed:", rollbackError.message);
      }
      throw error;
    }
