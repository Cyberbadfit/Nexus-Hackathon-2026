...
function parseUserMetadata(userOrStr, shouldDecrypt = true) {
  ...
  if (raw && raw.includes("|")) {
    ...
    branch = parts[4] || "CSE";
    section = parts[5] || "A";
    // Older registrations have seven segments and no year. New records keep
    // year immediately before approval status, so both formats stay readable.
    if (parts.length >= 8) {
      year = parts[6] || "";
      approvalStatus = parts[7] || "APPROVED";
    } else {
...
router.patch(
  ...
  async (req, res, next) => {
    try {
      ...
      const updatedMeta = `${meta.rollNo}|${meta.college}|${meta.phone}|${meta.course}|${meta.branch}|${meta.section}|${meta.year || ""}|APPROVED`;
...
router.post("/candidates", requireAdminAuth, async (req, res, next) => {
  try {
    ...
    const meta = `${encRoll}|${college || ""}|${encPhone}|${course || "B.Tech"}|${branch || "CSE"}|${section || "A"}|${year || ""}|${candidateStatus}`;
...
router.patch("/candidates/:id", requireAdminAuth, async (req, res, next) => {
  try {
    ...
    const targetYear = year !== undefined ? year : existingMeta.year || "";
    const targetBranch =
    ...
    const serializedMeta = `${targetRoll}|${targetCollege}|${targetPhone}|${targetCourse}|${targetBranch}|${targetSection}|${targetYear}|${targetStatus}`;
